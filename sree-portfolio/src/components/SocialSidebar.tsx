...
function Icon({ type }: { type: 'github' | 'linkedin' }) {
  if (type === 'github') {
    return (
...
export default function SocialSidebar() {
  return (
    <aside className="social-side" aria-label="Social profiles">
      <ul className="social-side__list">
        {socialLinks.map((s) => (
          <li key={s.url}>
            <a href={s.url} target="_blank" rel="noreferrer" className="social-side__link" aria-label={s.name}>
              <Icon type={s.icon} />
            </a>
          </li>
        ))}
      </ul>
      <span className="social-side__rule" aria-hidden />
    </aside>
  );
}
